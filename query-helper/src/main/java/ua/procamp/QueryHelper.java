...
public class QueryHelper {
    ...
    public <T> T readWithinTx(Function<EntityManager, T> entityManagerConsumer) {
        ...
        try {
            T apply = entityManagerConsumer.apply(entityManager);
            entityManager.getTransaction().commit();
            return apply;
        } catch (Exception e) {
            entityManager.getTransaction().rollback();
            throw new QueryHelperException("Exception during perform read operations in the scope of transaction." +
                    " Transaction is rolled back.", e);
        } finally {
            entityManager.close();
        }
    }
